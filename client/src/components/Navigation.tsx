...
import { LogOut, Sword, Users, Shield, Menu, X, LayoutDashboard } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side="right" className="bg-secondary/95 border-l border-white/10 w-[300px]">
            <div className="flex flex-col gap-6 mt-10">
              ...
              {user ? (
                <Button variant="destructive" className="w-full justify-start gap-2" onClick={() => { logout(); setIsOpen(false); }}>
                  <LogOut className="w-4 h-4" />
                  Logout
                </Button>
              ) : (
